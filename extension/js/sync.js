/**
 * sync.js
 * Google Drive Folder Sync & Local JSON Backup
 * Uses browser File System Access API (showDirectoryPicker) to sync directly
 * with your Google Drive desktop synced folder (e.g. G:\My Drive\Aura)
 */

import { store } from './store.js';

const IDB_NAME = 'aura_drive_db';
const IDB_STORE = 'handles';
const HANDLE_KEY = 'gdrive_dir_handle';
const SYNC_FILENAME = 'aura_sync.json';
...
async function saveDirHandle(handle) {
  try {
    ...
    console.warn('Could not save handle to IDB', e);
    return false;
  }
}
...
export class GoogleDriveSync {
  constructor() {
    this.dirHandle = null;
    this.folderName = localStorage.getItem('aura_gdrive_folder_name') || '';
    this.init();
  }
  ...
  async init() {
    ...
    if (this.dirHandle) {
      ...
      try {
        ...
        if (perm === 'granted') {
          console.log('Google Drive folder permission active:', this.folderName);
        }
      } catch (e) {
        console.warn('Folder permission check', e);
      }
    }
  }
  ...
  async selectFolder() {
    ...
    try {
      ...
      localStorage.setItem('aura_gdrive_folder_name', handle.name);
      await saveDirHandle(handle);

      // Immediately perform initial sync check
  ...
  async disconnectFolder() {
    this.dirHandle = null;
    this.folderName = '';
    localStorage.removeItem('aura_gdrive_folder_name');
    try {
      const db = await openIDB();
      const tx = db.transaction(IDB_STORE, 'readwrite');
  ...
  async saveToFolder() {
    ...
    try {
      // Verify or request permission if needed
  ...
  exportFile() {
    ...
    a.href = url;
    a.download = `aura_google_drive_backup_${new Date().toISOString().slice(0, 10)}.json`;
